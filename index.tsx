...
interface Plan {
  ...
  price: number;
  features: Feature[];
  detailedServices: { category: string; items: string[] }[];
  isPopular?: boolean;
}
...
const App = () => {
  ...
  const toggleDetails = (planId: string) => {
    setOpenDetails(prev => ({ ...prev, [planId]: !prev[planId] }));
  };
  ...
  const duplicateProposal = (record: ProposalRecord) => {
  ...
  const updatePlanField = (planIdx: number, field: keyof Plan, value: any) => {
    const newCats = [...categories];
    const cat = newCats.find(c => c.id === activeTab);
    if (cat) {
      (cat.plans[planIdx] as any)[field] = value;
      setCategories(newCats);
    }
  };

  const updateFeatureText = (planIdx: number, featureIdx: number, text: string) => {
    const newCats = [...categories];
    const cat = newCats.find(c => c.id === activeTab);
    if (cat) {
  ...
  const toggleFeatureInclusion = (planIdx: number, featureIdx: number) => {
    const newCats = [...categories];
    const cat = newCats.find(c => c.id === activeTab);
    if (cat) {
